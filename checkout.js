...
function checkoutSampler() {
    ...
    orderSummary += `<p id = "option"><strong>Drink:</strong> ${drink ? drink.value : "None"}</p>`;
    orderSummary += `<p id = "option"><strong>Meats:</strong> ${meats.length > 0 ? meats.map(meat => meat.value).join(", ") : "None"}</p>`;
    orderSummary += `<p id = "option"><strong>Sides:</strong> ${sides.length > 0 ? sides.map(side => side.value).join(", ") : "None"}</p>`;
